...
extension GithubUser {
    static func dummyUsers() -> [GithubUser]? {
        let data = getUsersData()
        do {
            let userList = try JSONDecoder().decode(
                [GithubUser].self, from: data)
            return userList
        } catch {
            return nil
        }
    }
    static func getUsersData() -> Data {
        let jsonString = """
        [
          {
            "login": "mojombo",
    ...
    static func dummyUser() -> GithubUser? {
        let data = getUserData()
        do {
            let userOne = try JSONDecoder().decode(
                GithubUser.self, from: data)
            return userOne
        } catch {
            return nil
        ...
        let jsonString = """
            {
              ...
              "updated_at": "2021-04-21T16:26:55Z"
            }
        """
        return Data(jsonString.utf8)
    }
}
extension GithubRepos {
    static func dummyRepos() -> [GithubRepos]? {
        let data = getReposData()
        do {
            let repoList = try JSONDecoder().decode(
                [GithubRepos].self, from: data)
            return repoList
        } catch {
            return nil
        }
    }
    static func getReposData() -> Data {
        let jsonString = """
        [
        {
        "id": 26899533,
